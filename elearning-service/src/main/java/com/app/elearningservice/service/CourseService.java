...
public class CourseService {
    ...
    public boolean registerCourse(Long courseId, Long userId) {
        return writeDb.update("""
                insert into time_table(user_id, course_id) VALUES (:userId, :courseId)
                """, new MapSqlParameterSource().addValue("userId", userId).addValue("courseId", courseId)) > 0;
    }

    public Course findById(Long courseId) {
        return writeDb.queryForObject("SELECT * FROM courses WHERE course_id = :courseId", Map.of("courseId", courseId), BeanPropertyRowMapper.newInstance(Course.class));
    }

    @Transactional
    public void delete(long courseId) {
        writeDb.update("update courses set status = 'inactive' where course_id = :courseId", Map.of("courseId", courseId));
    }
    ...
    public void save(CoursePayload payload, String thumbnail) {
        ...
        var param = new MapSqlParameterSource().addValue("name", payload.getName()).addValue("description", payload.getDescription()).addValue("start_date", payload.getStartDate()).addValue("end_date", payload.getEndDate()).addValue("thumbnail", thumbnail);
    ...
    public PagingContainer<Course> findAll(int page, int size, String key, Long userId) {
        var sql = """
                 select c.course_id,
                        ...
                        if(tt.status = 'approved', 2, IF(tt.status = 'pending', 1, 0)) is_join,
                        is_register
                from courses c
                 ...
                 where true
                     ...
                     and c.name like :key;
                """;
        var param = Map.of("key", "%" + key + "%", "user_id", userId);
