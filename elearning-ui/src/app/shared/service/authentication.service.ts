...
export class AuthenticationService {
  ...
  public get isTeacher(): boolean {
    return this.currentUser.value.isTeacher;
  }
  ...
  redirectLogin() {
    this.router.navigate([CONSTANT.studentPath])
      .then(_ => window.location.reload());
  }

  redirectHome() {
    this.router.navigate([this.currentUser.value.isAdmin ? CONSTANT.adminPath : CONSTANT.studentPath])
      .then(_ => window.location.reload());
  }
